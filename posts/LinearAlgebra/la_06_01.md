...
Let $$\textbf{u},\textbf{v}$$, and $$\textbf{v}$$ be vectors in $$\mathbb{R}^n$$, and let $$c$$ be a scalar. Then
...
If $$\textbf{v}$$ is in $$\mathbb{R}^n$$, with entries $$v_1,\cdots, \v_n$$, then the square root of $$\textbf{v}\cdot \textbf{v}$$ is defined because  $$\textbf{v}\cdot \textbf{v}$$ is nonengative.
...
* If we identify $$\lVert \textbf{v} \rVert$$ with a geometric point in the plane, 
  ...
  *the standard notion* of **the length of the line segment** of from the origin to $$\textbf{v}$$.
...
* The two lines shown in the figure below are **geomatrically perpendicular** 
...
* The **zero vector is orthgonal to every vectors** in $$\mathbb{R}^n$$ becuase $$\textbf{0}^T \textbf{v} = 0$$ for all $$\textbf{v}$$.
...
$$
\textbf{ur} \cdot \textbf{v} = \lVert \textbf{u} \rVert \lVert \textbf{v} \rVert \cos{\theta} \tag{2}
$$
...
$$
\begin{align*}
\Vert \textbf{u} \Vert \Vert \textbf{u} \Vert \cos{\theta} &= \frac{1}{2} \left[ \Vert \textbf{u} \Vert^2 \Vert \textbf{v} \Vert^2 - \Vert \textbf{u}-\textbf{v} \Vert^2 \right] \\
&= \frac{1}{2} \left[ u_1^2 + u_2^2 +v_1^2 +v_2^2 - (u_1-v_1)^2 - (u_2-v_2)^2 \right] \\
&= u_1 v_1 +u_2v_@ \\
&= \textbf{u}\codt\textbf{v}
\end{align*}
$$
