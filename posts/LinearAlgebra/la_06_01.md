...
If $$\textbf{u}$$ and  $$\textbf{v}$$ are vector in $$\mathbb{b}$$,
...
The number $$\textbf{u}^T\textbf{v}$$ is called the **inner product** of $$\textbf{u}$$ and $$\textbf{v}$$, and it is written as $$\textbf{u} \cdot \text{v}$$.
...
$$
(c_1 \bf{u}_1  +\cdots +c_p \bf{u}_p) \cdot \bf{w} = c_1 (\bf{u}_1 \cdot \bf{w}) + \cdots + c_p (\bf{u}_p \cdot \bf{w})
$$
...
$$
\lVert c \bf{v} \rVert = |c| \lVert \bf{v} \rVert
$$
...
$$
\bf{ur} \cdot \bf{v} = \lVert \bf{u} \rVert \lVert \bf{v} \rVert \cos{\upvartheta} \tag{2}
$$
...
In statistics, the value of $$\cos{\upvartheta}$$ defined by (2) for suitable vectors $$\bf{u}$$ and $$\bf{v}$$ is called a correlation coefficient.
