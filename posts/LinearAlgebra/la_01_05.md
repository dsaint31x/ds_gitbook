...
The homogeneous equation $$A\bf{x}=\bf{0}$$, the important equation is whether there exists a **nontrivial solution**, that is, a nonzero vector $$\bf{x}$$ that satisfies $$A\bf{x}=\bf{0}$$.
...
The equation of the form $$\bf{x}=s\bf{u} +t\bf{v} (s,t in \mathbb{R})$$ is called a **parametric vector equation** of the plane.

In Example 1, the equation $$\bf{x}=x_3\bf{v}$$ (with $$x_3$$ free), or $$\bf{x}=t\bf{v}$$ (with $$t$$ in $$\mathbb{R}$$),
...
$$
\bf{x}=t\bf{v} \quad (t \text{ in }  \mathbb{R}) \tag{4}
$$

[with the same $$\bf{v}$$ that appears in (3)].

Thus the solutions of $$A\bf{x}=\bf{b}$$ are obtained by adding
the vector $$\bf{p}$$ to the solutions of $$A\bf{x}=\bf{0}$$.
...
We say that $$\bf{v}$$ is translated by $$\bf{p}$$ to See the following figure.
...
,although the solution set will be larger than a line when there
are several free variables.

## Theorem 6

Suppose the equation $$A\bf{x}=\bf{b}$$ is consistent for some given $$\bf{b}$$, and let $$\bf{p}$$ be a solution. 

Then the solution set of $$A\bf{x}=\bf{b}$$ is the set of all vectors of the form $$\bf{w}=\bf{p}+\bf{v}_h$$, where $$\bf{v}_h$$ 
