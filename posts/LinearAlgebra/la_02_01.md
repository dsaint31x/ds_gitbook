...
* If this vector is then multiplied in turn by a matrix $$A$$, the resulting vector is $$A(B\textbf{x})$$. See the Fig. 2 below.
...
\end{align}
$$
* The vector $$A(B\textbf{x})$$ is a linear combination of the vectors $$A\textbf{b}_1+\cdots+A\textbf{b}_p$$, using the entries in $$\textbf{x}$$ as weights.
...
That is,
$$AB = A\begin{bmatrix}\textbf{b}_1, \cdots, \textbf{b}_p \end{bmatrix} = A \textbf{b}_1, \cdots, A \textbf{b}_p$$
...
4. $$r(AB)=(rA)B =A(rB)$$ for ans scalar $$r$$.
