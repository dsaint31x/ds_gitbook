...
$$
[\textbf{x}]_C = P_{C \leftarrow B}[\textbf{x}]_B \tag{4}
$$

The columns of $$P_{C \leftarrow B}$$
...
The columns of $$P_{C \leftarrow B}$$ are linearly independent 
because they are the coordinate vectors of the linearly independent
set $$B$$.

Since $$P_{C \leftarrow B}$$ is square, it must be invertible, 
by the Invertible Matrix Theorem. Left-multiplying both sides of equation (4)
by $$\left(P_{C \leftarrow B}\right)^{-1}$$ yields

$$
\left(P_{C \leftarrow B}\right)^{-1} [\textbf{x}]_C = [\textbf{x}]_B
$$

Thus $$\left(P_{C \leftarrow B}\right)^{-1}$$
is the matrix that converts $$C$$-coordinates
into $$B$$-coordinates. That is,

$$
\left( P_{C \leftarrow B}\right)^{-1} = P_{B \leftarrow C} \tag{6}
$$
...
* If $$B=\left\{ \textbf{b}_1, \cdots, \textbf{b}_n \right\} $$
  ...
  in $$B$$. In this case, $$P_{\epsilon \leftarrow B}$$
