...
Suppose $$\mathcal{B}=\left\{\textbf{b}_1, \textbf{b}_2 \right\}$$ is a basis for $$V$$ and $$\mathcal{C}=\left\{\textbf{c}_1,\textbf{c}_2,\textbf{c}_3\right\}$$ is a basis for $$W$$. Let $$T: V \leftarrow W$$ be a linear transformation with the property that
...
$$
[T(\textbf{b_1})]_\mathcal{C} = \begin{bmatrix} 3 \\ -2 \\ 5 \end{bmatrix} 
\text{ and } 
[T(\textbf{b_2})]_\mathcal{C} = \begin{bmatrix} 4 \\ 7 \\ -1 \end{bmatrix}  
$$
...
In the common case where $$W$$ is the same as $$V$$ and the basis $$\mathcal{C}$$ is the same as $$\mathcal{B}$$, the matrix $$M$$ in (4) is called the matrix for $$T$$ relative to $$\mathcal{B}$$, or simply the
$$\mathcal{B}$$-matrix for $$T$$, and is denoted by $$[T]_\mathcal{B}$$.
...
The mapping $$ T: \mathbb{P}_2 \leftarrow \mathbb{P}_2 $$ defined by 
$$
