...
Here $$\textbf{y}$$ is the coordinate vector of $$\textbf{y}$$ relative to the basis of $$\mathbb{R}^n$$ determined by the columns of $$P$$. (See Section 4.4.)
...
where $$A$$ is an invertible $$2 \time 2$$ symmetric matrix, 
...
## Definition : Qaudaratic Form

A quadratic form Q is:

a. positive definite if $$Q(\textbf{x}) > 0$$ for all $$\textbf{x} \ne 0$$
b. negative definite if $$Q(\textbf{x}) < 0$$ for all $$\textbf{x} \ne 0$$
c. indefinite if $$Q(\textbf{x})$$ assumes both positive and negative values.
